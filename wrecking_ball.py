...
parser.add_argument('--json_file', help='Benchmark file', type=str, default='fixtures/3D/chain/wrecking-ball.json')
parser.add_argument('--create_video', help='Create video', type=int, default=0)
parser.add_argument('--shift_mag', help='Shift the collision/visual (due to OBJ file not centered) values -1=negative shift, 0 = no shift, 1 = positive shift', type=int, default=1)
...
def objectFromObj(objFileName, mass=1, meshScale=[1,1,1]):
  ...
  if mass==0:
     col_flags=p.GEOM_FORCE_CONCAVE_TRIMESH #| p.GEOM_CONCAVE_INTERNAL_EDGE
  
  print("testing objFileName=",objFileName)
  if (objFileName=="meshes/wrecking-ball/ball_split.obj"):
     mass = 200
  if (objFileName=="meshes/cube.obj"):
    mass = 0.1
...
p.setPhysicsEngineParameter(enableSAT=1, numSolverIterations=200)
...
for body in rigid_body_problem['rigid_bodies']:
  ...
  p.changeDynamics(body_id,-1,lateralFriction=0.503, frictionAnchor = 1)
  pos = [0,0,0]
  print("body=",body)
  if 'position' in body:
    pos = body['position']
...
cameraYaw = -180.
sim_frame=0.
p.resetDebugVisualizerCamera(cameraDistance=15,cameraYaw=cameraYaw,cameraPitch=cameraPitch,cameraTargetPosition=[-1,10,10])
